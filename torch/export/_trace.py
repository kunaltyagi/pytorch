    ...
    for node in traced_module.graph.nodes:
        if node.op == "get_attr":
            ...
            if attr_name in param_buffer_table:
                attr = getattr(traced_module, attr_name)
                ...
                # longer exists.
                fqn = param_buffer_table[attr_name].replace(".", "_")
                delattr(traced_module, attr_name)

                if isinstance(attr, torch.nn.Parameter):
                    traced_module.register_parameter(fqn, attr)
                else:
